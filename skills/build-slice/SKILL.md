...
Bootstrap (slice-095 only): slice-095 authors SVW-1; at slice-095's own Step 6 the audit runs against the routed worktree and MUST exit 0 (self-application discharge — the 10 append sites routed through `vault_edit append` + 11 RMW/project-open sites exempt-marked). Every slice after 095 inherits a self-gating SVW-1.
