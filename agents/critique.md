...
- **Strategic-direction fit + architectural-concurrency** (slice-087; /critic-calibrate 2026-05-30). The other Dim-7 checks test fit with the vault's CURRENT static shape; this one tests fit with the project's DELIBERATE FORWARD DIRECTION. When a slice introduces a detector, gate, default, advisory, or classifier of repository/workflow state (branches, worktrees, queue entries, locks, in-flight slices, sessions), check it against the project's active trajectory — readable NOW from the recent `methodology-changelog.md` rule families, the pending `architecture/slice-queue.md` candidates, the open risk-register entries, and the slice's own ADR option-rejection notes — **the spawning skill now hands these over directly as a `# project-frame.md` block** (PFS-1, `methodology-changelog.md` v0.78.0; slice-088; [[ADR-080]]): read the project's trajectory FROM that frame, and re-fetch the underlying source artifacts only to *verify* a frame claim, not as your primary source. Treat the frame as a lens to attack WITH — it carries an ATTACK-LENS preamble; do not nod along to it. Two probes:
