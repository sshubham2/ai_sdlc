...
- **Methodology-surface RULE-ID + entry-pin obligation** (MEPD-1; slice-039; a checklist pass run SEPARATELY from the deep-dive — the deep-dive consistently crowds out this mechanically-derivable item, so it gets its own explicit pass). When a slice changes behaviour on an in-house methodology surface (`skills/*/SKILL.md`, `agents/*.md`, `tools/**/*.py`, `methodology-changelog.md`, the in-house audits), the Critic MUST verify the slice discharges the changelog/RULE-ID obligation along EXACTLY ONE of two branches and say which:
  - **(a) rule path** — a minted RULE-ID, a `tests/methodology/test_methodology_changelog.py::test_v_0_NN_0_<rule>_entry_present_in_repo_and_installed` entry-pin for the new version, AND the atomic 4-part PMI-1 bump (`VERSION` + `~/.claude/ai-sdlc-VERSION` + `plugin.yaml.version` + the forward-synced `~/.claude/methodology-changelog.md`); OR
  - **(b) documented-why-none path** — an explicit, written rationale for *why no rule/entry is required*, **verified against the actual `tests/methodology/test_methodology_changelog.py` enforcing assertion** (open the test and read what it actually requires — e.g. only a literal `Rule reference` line vs a minted ID). The rationale MUST NOT rest on a Builder-asserted "slice-NNN did/didn't X" prior-slice precedent: confirm the precedent against the enforcing artifact, never the claim (a false precedent has been rubber-stamped by BOTH Critic layers — slice-032 m1). A behaviour-changing methodology slice that silently carries *neither* branch is a Major (the slice-032 DEVIATION-1 "auto-blessed no entry" anti-pattern; the slice-034 M-add-1 first-Critic miss the DR-1 meta-Critic had to catch). Apply this mechanically — its whole point is that the deep-dive does not reliably reach it.
- **Strategic-direction fit + architectural-concurrency** (slice-087; /critic-calibrate 2026-05-30). The other Dim-7 checks test fit with the vault's CURRENT static shape; this one tests fit with the project's DELIBERATE FORWARD DIRECTION. When a slice introduces a detector, gate, default, advisory, or classifier of repository/workflow state (branches, worktrees, queue entries, locks, in-flight slices, sessions), check it against the project's active trajectory — readable NOW from the recent `methodology-changelog.md` rule families, the pending `<vault>/slice-queue.md` candidates, the open risk-register entries, and the slice's own ADR option-rejection notes — **the spawning skill now hands these over directly as a `# project-frame.md` block** (PFS-1, `methodology-changelog.md` v0.78.0; slice-088; [[ADR-080]]): read the project's trajectory FROM that frame, and re-fetch the underlying source artifacts only to *verify* a frame claim, not as your primary source. Treat the frame as a lens to attack WITH — it carries an ATTACK-LENS preamble; do not nod along to it. Two probes:
  - **(a) Direction-fit** — does the slice stay correct when the project's deliberate direction is in effect (e.g. the parallel-slice family PSQ-1 / PSQ-2 / PSQ-3 / BRANCH-2, or whatever the trajectory names as active)? A rejection note in the slice's OWN ADR ("Option X breaks PSQ-1/PSQ-2 …") that also applies to a softer / shipped variant of the SAME mechanism is a Blocker — generalize the warning from the rejected option to the shipped one; do not let it stay quarantined to the option that was declined.
  - **(b) Architectural concurrency** (distinct from Dimension 2's RUNTIME concurrency of two users hitting an endpoint): under the project's parallel-execution model, is the NORMAL operating state one where MANY instances of the detected thing co-exist (N concurrent unmerged `slice/*` branches + worktrees is normal under BRANCH-2 + PSQ)? If so, a "flag all / treat-each-as-anomaly" design cry-wolfs on every legitimately-in-flight instance → R-7 silent-disable + actively undermines the strategic direction. Require the slice to carve out the legitimately-in-flight normal state, not merely the narrow self-reference case. Concrete miss: slice-087 flagged ALL unmerged `slice/*` as 'stranded'; both Critic layers missed that this fires on every parallel in-flight slice (the user caught it).
...
- **Shippability-catalog consumer-reference propagation** — N=2 sub-clause (slice-013 N=1 reactive-catch at /validate-slice Step 5.5 + slice-014 N=2 proactive-application at /build-slice Phase 4 pre-/validate-slice); peer cross-reference: none — operates at the cross-Phase consumer-reference-propagation discipline level, distinct from the meta-level recursive-self-application discipline and the Edit-discipline-level entry-pin-vs-PMI-1-gate semantics conflation above. When a slice supersedes a test function name in `tests/methodology/*.py` (via PMI-1 versioned-gate supersession, PMI-1 structural-invariant supersession applied at the structural-invariant level, or any other rename-driven supersession discipline) at /build-slice Phase 1, downstream consumers of that test function name in `<vault>/shippability.md` (the catalog of critical-path commands run at /validate-slice pre-finish) carry stale references unless the same /build-slice block (before /validate-slice catalog run) explicitly propagates the rename across all consumer rows. Two distinct sub-modes:
  ...
  - **Proactive-application mode** — within the same /build-slice block as the supersession Edit and BEFORE the /validate-slice catalog run (typically Phase 4 or Phase 5 of the /build-slice plan, not necessarily the same numbered Phase as the supersession Edit itself), the slice authoring a supersession explicitly scans `<vault>/shippability.md` for ANY rows referencing the deleted/renamed test name AND updates each row's pytest command in-line. Concrete catch: slice-014 N=2 (proactive — supersession Edit at Phase 1c; row 13 pytest command propagated from `_at_0_28_0` to `_invariant` at Phase 4 BEFORE running shippability catalog at /validate-slice; 14/14 PASS no regressions). The slice-013 reactive lesson was applied PROACTIVELY at slice-014 — **N=2 promotion threshold MET** (slice-013 reactive + slice-014 proactive across two consecutive cross-cutting-tooling slices).

  When reviewing a slice that supersedes any test function name in `tests/methodology/*.py` (PMI-1 versioned-gate, PMI-1 structural-invariant test, or any other rename-driven supersession), the Critic SHOULD: (1) require the design.md phase plan to enumerate the supersession Phase AND the shippability.md propagation Phase as distinct, named steps within the same /build-slice block; (2) require the propagation Phase to execute in the same /build-slice block as the supersession Edit and BEFORE the /validate-slice catalog run (not deferred to /validate-slice reactive time or "fixed-when-fails"); (3) require a pre-Phase empirical scan of `<vault>/shippability.md` for ALL consumer references to the test name being superseded (mirrors slice-014 row 13 proactive scan); (4) flag absent or aspirational propagation plans as Major findings — the slice-013 reactive miss empirically demonstrated that Critic-stack layers 1 + 2 (first Critic + /critique-review meta-Critic) BOTH missed the lesson generalization to shippability.md, so codification at Dim 9 sub-class level is the canonical mitigation.

- **Runtime-prerequisite completeness on proposed fixes** — N=3 sub-clause (slice-013 M-add-1 sibling-test grep + slice-014 M-add-1 missing-imports + slice-015 M-add-1 audit-allowlist non-membership); peer cross-reference: none — operates at the fix-introduces-new-runtime-prerequisite-gap level, distinct from RSAD-1 (catches own-draft defects in slice's own prose) and SCPD-1 (catches cross-Phase consumer propagation). When the Critic proposes a fix (whether at /critique or /critique-review), the fix may introduce a runtime prerequisite that surrounding tooling/context doesn't satisfy. The first Critic correctly identifies the design-semantic defect AND a structurally sound fix; the runtime-prerequisite gap is what the second Critic (DR-1) has historically caught. To close that loop at first-Critic time, when proposing any fix involving (a) a NEW symbol used in a Python test body — verify the module imports that symbol; (b) a NEW status string / enum value / token introduced by the fix — verify the relevant audit/parser/allowlist accepts it (e.g., `tools/test_first_audit.py` `_ALLOWED_STATUSES`); (c) a NEW end_anchor / start_anchor / body-bound for a test — grep the test file for sibling tests sharing the same anchor pair and propose tightening them in lockstep. Concrete misses: slice-013 M-add-1 (sibling-test grep absent at first-Critic); slice-014 M-add-1 (missing `import pytest` + `import ast` at canonical regression-test body); slice-015 M-add-1 (`WRITTEN-AS-EDIT` not in `_ALLOWED_STATUSES`). Proposed rule-ID: `RPCD-1` per -D-suffix convention N=3 stable.

- **Fix-block-completeness discipline** — N=10-cumulative-cross-instance / N=4-distinct-slice sub-clause (slice-020 M-add-1 N=1 + slice-021 M-add-1-rerun/2-rerun/3-rerun N=3 + slice-022 M-add-2/3 N=2 + slice-023 M-add-1/2/3/4 N=4); peer cross-reference: this is the **temporal extension** of the `Tooling-doc-vs-implementation parity` sub-clause's design.md-mechanical-tables-vs-canonical-inventory body (i.e., CCC-1 v1.1) — that sub-clause catches mechanical-table-vs-inventory drift on the initial draft against an EXTERNAL canonical inventory; this sub-clause catches drift WITHIN-slice across the slice-authoring file set, and is the structural-runtime sibling to the `Runtime-prerequisite completeness on proposed fixes` sub-clause (i.e., RPCD-1). When a slice authors claims (file paths, numeric counts, ADR IDs, test function names, rule IDs, anchor literals) across multiple slice-authoring files — `mission-brief.md` + `design.md` + `ADR-NNN` + `milestone.md` — those claims drift between sites at draft time, and a Builder's single-site ACCEPTED-FIXED disposition at /critique-disposition triage frequently fails to propagate to all sibling sites carrying the same claim. Three sub-modes — two on the temporal axis ((a)/(b)) plus one on the orthogonal SCOPE axis ((c)):
  - **Sub-mode (a) Original-draft cross-file consistency** — first-Critic catch at /critique time. When a slice authors a claim across multiple files, the Critic MUST grep the claim's anchor substring across all slice-authoring files and verify the claim is identical at every site. Concrete misses: slice-022 N=1 (NEW class — sub-mode (a) inaugural instance) + slice-023 N=2 (B5 17/17/17 count drift across mission-brief AC #2 + design.md L120/121/130/131 + L225 + L344; B3 `tests/decisions/` ADR-pin path drift across mission-brief + design.md) = N=3 cumulative under sub-mode (a) per slice-023 reflection Pattern 4. The `mission-brief.md` surface is the most commonly under-grepped site (the Critic-stack historically scopes grep to design.md + ADR and treats mission-brief / milestone as passive logs rather than load-bearing claims surfaces — slice-021 aggregated lesson).
  - **Sub-mode (b) Post-ACCEPTED-FIXED sibling-sweep** — meta-Critic catch at /critique-review time. When the Builder applies an ACCEPTED-FIXED fix at one site at /critique-disposition triage, the meta-Critic MUST grep the anchor substring across all slice-authoring files post-fix to verify the fix-block propagated to every sibling site. Concrete misses: slice-020 M-add-1 ("same class as B3 recurring in fix block" — one-site B3 fix missed sibling occurrences); slice-021 M-add-1-rerun/2-rerun/3-rerun (three fix-block-completeness recursions where the Builder's "harmonized at N surfaces" claim left ~1 residual stale site per fix-block — milestone.md L43/L46 + design.md L197/L209/L211 un-propagated); slice-022 M-add-2/3 (m2 second sibling site un-propagated + m3 count-drift between two design.md sites); slice-023 M-add-1/2/3/4 (post-fix sibling sweep gaps including M5 test-file split sibling sites at design.md L194 + mission-brief.md L54 + ADR-021 L73).
  ...
  When reviewing a slice's design at /critique (sub-mode a), OR reviewing the Builder's ACCEPTED-FIXED dispositions at /critique-review (sub-mode b), the Critic SHOULD: (1) for every numeric count, file path, ADR ID, rule ID, test function name, or anchor literal asserted in the slice's drafts, grep that anchor substring across the full slice-authoring file set `{mission-brief.md, design.md, ADR-NNN, milestone.md}` and enumerate every site; (1b) when the slice declares a counted-set cardinality change (sub-mode c), extend the grep beyond the slice-authoring file-set to the WHOLE repo — every `== N`, `len(...) == N`, `"N-element"`, `"set to N"`, and INSTALL.md count literal referencing that set — and enumerate every site; (2) verify the claim is byte-identical at every enumerated site (sub-mode a) OR that the Builder's fix-block propagated the new value to every enumerated site (sub-mode b); (3) treat any ≥3-site multi-surface ACCEPTED-FIXED sweep as high-risk for a residual stale sibling and explicitly re-grep post-fix; (4) flag absent or aspirational "harmonized at N surfaces" claims that lack a grep-verified site enumeration as Major findings — the slice-020/021/022/023 evidence empirically demonstrates Critic-stack layers 1 + 2 missed this class N=10 cumulative times before codification, so codification at Dim 9 sub-clause level is the canonical mitigation. Proposed rule-ID: `FBCD-1` per -D-suffix convention N=7 stable (RSAD-1 + EPGD-1 + SCPD-1 + RPCD-1 + TPHD-1 + LAYER-EVID-1 + FBCD-1).

- **Phantom test-file citation discipline** — N=2-distinct-slice sub-clause (slice-023 B4 `test_row_*.py`-convention citation caught at /critique + slice-024 `test_shippability_catalog.py` citation that survived 19 Critic-stack findings + BOTH Critic layers and died only at /validate-slice Step 5.5 real-command execution); peer cross-reference: this is the **execution-reachable structural sibling** to the `Runtime-prerequisite completeness on proposed fixes` sub-clause (RPCD-1) — RPCD-1 catches a fix introducing a NEW symbol/status/anchor the surrounding tooling does not satisfy; this sub-clause catches a slice citing a test-file *path* that does not exist on disk. The Critic-stack reviews TF-1 plan `Test path` cells and shippability `Command`-cell pytest targets statically for name-harmonization (TPHD-1), status, and cross-file consistency (FBCD-1) but NO static surface verifies "does the cited test file exist when the stated command runs" — that question is reachable only at command execution, which is why slice-024's phantom citation survived raw-finding-count 19 + the full two-layer Critic stack. Two distinct sub-modes covering the two citation surfaces, each extended from FILE existence to test-FUNCTION existence by the **Function-level layer (PTFFD-1, slice-037)** — function-level phantom-citation is a distinct N=3-distinct-slice Critic-stack blind spot (slice-025 AC3 + slice-026 AC5 + slice-027 B1):
  - **Sub-mode (a) TF-1-plan-path existence** — `/build-slice` Step 6 strict-pre-finish surface. When a slice's mission-brief TF-1 plan row reaches PASSING, its `Test path` cell MUST resolve to a file that exists on disk; `tools/test_first_audit.py --strict-pre-finish` emits `missing-test-path-file` for any PASSING row whose resolved path is absent (gated on `row.status == "PASSING"` so a still-PENDING test-first row's not-yet-created file is reported once by the pre-existing `non-passing-pre-finish` loop, never doubled). Concrete miss pre-codification: slice-023 B4 — `tests/methodology/test_row_*.py` convention cited in the TF-1 plan; no such file/convention exists; caught reactively at /critique only because a human-review pass happened to grep it.
  - **Sub-mode (b) shippability-Command-cell-path existence** — `/validate-slice` Step 5.5 pre-catalog surface. Every `tests/<...>.py` token after the `pytest` keyword in an `<vault>/shippability.md` `Command` cell MUST resolve to an existing file (after markdown-backtick strip + `::`-selector split); `tools/shippability_path_audit.py` gates the Step 5.5 catalog run on this BEFORE execution so a phantom citation is reported as one clear violation instead of N confusing per-row pytest "file not found" FAILs. Concrete miss pre-codification: slice-024 — `tests/methodology/test_shippability_catalog.py` cited at 3 sibling sites; no such file exists; survived 19 Critic-stack findings + first-Critic + meta-Critic and was caught only when /validate-slice Step 5.5 actually ran the catalog command (slice-024 reflection "Corrected").
  - **Function-level layer (PTFFD-1, slice-037; ADR-038)** — BOTH sub-modes extend from "the cited test FILE exists" to "the cited test FUNCTION exists in it". When the FILE exists but the cited test FUNCTION is absent, `tools/test_first_audit.py --strict-pre-finish` emits `missing-test-function` and `tools/shippability_path_audit.py` emits a `kind="missing-test-function"` PhantomCitation. The function name resolves function-column-first then falls back to the `Test path` / selector `::`-tail (`[param-id]` stripped); a non-identifier / prose value (e.g. slice-034's real `(full existing module — non-regression)` TF-1 row) is NOT a checkable name and degrades to FILE-level-only — never a false-positive. An unparseable / non-Python / unreadable cited file is **skip-with-note** (ADR-037): NO violation, but the skip is rendered (not silent) — a false-FAIL on a parse failure would halt the pipeline gate, the strictly worse audit failure mode. Concrete misses pre-codification (the function-level Critic-stack blind spot, N=3-distinct-slice): slice-025 AC3 + slice-026 AC5 + slice-027 B1 — each a phantom test *function* in an otherwise-present file, each missed by BOTH Critic layers and caught only by a downstream gate. **PTFFD-1** is a NEW minted `-D`-suffix rule-ID that refines PTFCD-1 in place — rule-ID lineage preserved, supersedes nothing — mirroring the TFFL-1↔TF-1 / EOL-DRIFT-1↔CAD-1 in-place-mint precedent (NOT a `PTFCD-1 v1.1` version label: the `vN.N` label is reserved for the NON-`-D` audit-gate naming class).

  When reviewing a slice that authors TF-1 plan rows OR shippability catalog `Command` cells, the Critic SHOULD: (1) for every `Test path` cell and every `tests/<...>.py` token in a shippability `Command` cell, grep-or-stat the cited path against the on-disk repo tree and flag any that does not exist (excepting a test-first row legitimately PENDING because the slice itself creates that file — verify the slice's own plan creates it, do not assume), AND for every TF-1 `Test function` cell / shippability `::`-selector whose FILE exists, verify the cited test FUNCTION is defined in that file (PTFFD-1 function-level layer — `def`/`async def` at any nesting depth; a prose/non-identifier value degrades to FILE-level-only, an unparseable file is skip-with-note); (2) treat a citation of a *convention* or *pattern* (`test_row_*.py`, `test_<slice>_*.py`) rather than a concrete extant path as a phantom-citation Blocker — conventions are not files; (3) recognise that raw-finding-count is NOT a completeness signal for this class (slice-024 N=19 + 2 layers missed it) and therefore apply this check mechanically rather than relying on having "looked carefully"; (4) flag absent existence-verification as a Major finding — slices 023/024 empirically demonstrate the static Critic stack cannot reach this class, so codification at Dim 9 sub-clause level plus the two audit gates (`test_first_audit.py` strict-pre-finish + `shippability_path_audit.py` pre-catalog) is the canonical mitigation. Proposed rule-ID: `PTFCD-1` (FILE-level) + `PTFFD-1` (function-level, slice-037) per -D-suffix convention N=8 → N=9 stable (RSAD-1 + EPGD-1 + SCPD-1 + RPCD-1 + TPHD-1 + LAYER-EVID-1 + FBCD-1 + PTFCD-1 + PTFFD-1).

- **Audit-parse-rule empirical-execution discipline (scope: any minted regex / glob / pathspec / parse rule)** — N=3-MISS + N=1-CATCH original sub-clause (slice-030A R-5 CRLF + slice-031 R-6/R-7 + slice-033 R-7 N+1 = three first-Critic MISSES of an audit's OWN parse-rule misbehaving on a real artifact; slice-034 M1 = the counter-example CATCH that proved the class IS Critic-reachable when the Critic *executes* rather than reasons); **scope extended at the 2026-05-28 calibration run (window 061–075) to cover non-audit minted regex/pathspec/glob: slice-069 M5 INCLUDE pathspec over-match + slice-070 AC#3 `.gitignore` dotfile blind-spot + slice-071 B1 `_PATH_SHAPED_RE` broken regex were three first-Critic MISSES of the same empirical-execution discipline applied to non-audit surfaces; slice-074 pass-2 demonstrated gold-standard rigor (B1/M1/M2 caught by APED-1 execution on the bash-extraction regex) confirming the broader class IS Critic-reachable when generalized.** Peer cross-reference: structural sibling of the `Phantom test-file citation discipline` sub-clause (both are reachable only by running something against the real artifact, not by static reading) — but distinct: PTFCD-1/PTFFD-1 is path/function *existence*, this is parse-rule *behaviour*. The recorded belief through slice-033 was "the dual-Critic stack structurally cannot reach the audit-parsing-its-own-artifact interaction; only the BC-PROJ-4 pre-finish real-artifact run can" — slice-034 M1 falsified that *at /critique design-time* by running the proposed `(true|false)\b.*$` regex against `false-positive` and catching a narrower R-7 self-violation. The discipline generalises that one success into a standing obligation. When a slice changes an audit's own parse rule — a regex, a field-line matcher, a token splitter, a status/enum acceptor, a backtick/`;`-segment splitter, an AST-shape predicate — design-time *reasoning* about what the rule will do is insufficient and has a documented N=3 miss record; the rule must be **executed**.

  When reviewing a slice that mints or modifies **any parse-time-evaluated pattern** — a `tools/**` audit's parse rule (or a SKILL.md prose rule an audit enforces), an AC-row test's regex/glob literal, a git `pathspec` (`:(exclude)<...>`, `:(glob)<...>`), a `Glob`/`Grep` pattern in skill prose, or any other regex/glob/pathspec minted in the slice's mission-brief / design.md / new ADR — the Critic SHOULD: (1) **Bash-execute** the changed pattern — directly, or via a throwaway `python -c` harness, or by running the audit on a crafted fixture — against an adversarial battery covering at minimum: the slice's **own mission-brief / design field-lines** as literal inputs; a **trailing-annotation** variant (`**Field**: true  (per X — …)` — the R-7 class); a **substring-collision** variant (`false-positive`, `true.`, `disabled; note` — the over-match class); the **empty / absent** input (default-off vs loud-malformed); a **CRLF** vs LF variant (the R-5 class) when the rule consumes file bytes; a **dotfile / hidden-path input** (`.gitignore`, `.env`, `.claude/`) when the rule scopes a glob or pathspec against the repo tree (the slice-070 `.gitignore` class); and a **representative real-corpus input from the actual data domain the rule will run against** (e.g., for an INCLUDE-direction pathspec, the actual file tree the pathspec will match — the slice-069 class); (2) file a **Blocker** on any observed silent-disable, default-off-on-malformed, false-FAIL on a legitimate input, over-match on a colliding substring, **or under-match on a legitimate-but-edge-case input the rule's domain includes (dotfiles, deeply-nested paths, paths with `-` / `_` / `.` boundaries)** — these are not hypotheticals once executed, they are reproductions; (3) state explicitly in the finding (or in the dimension's no-findings note) that the battery was **executed, not reasoned** — quote the command and its observed output, so the Builder can re-run it rather than trust an LLM summary (slice-032/036 "the Critic reasons about the claim, not the artifact" law applied to the Critic's own parse-rule review); (4) treat absence of an executed battery on an audit-parse-rule-changing slice as a Major — the slice-030A/031/033 evidence empirically demonstrates the static Critic stack misses this class N=3, so codification at Dim 9 sub-clause level (plus the BC-PROJ-4 pre-finish real-artifact backstop) is the canonical mitigation, and slice-034 proved the *executed* check closes it at design-time; (5) when the FINDING ITSELF spec's an APED-1-execution discipline FOR THE PROPOSED FIX — e.g., a Critic finding proposing a new regex pin AND citing "this must be APED-1-executed at build time" — the Critic MUST execute the proposed regex/pathspec/glob against the actual target prose AT /critique TIME, not defer to build time. Specifying the discipline without applying it to the proposed literal is the design→code translation gap N=15 cumulative class (slice-070 `.gitignore` post-meta-fix regex blind-spot + slice-076 M3 design-mandated atomicity + slice-076 M3 design-mandated defense-in-depth gate + slice-077 `_resolve_milestone_path` scan-root + slice-078 Pin #2 over-broad shape). The whole point of APED-1 is that "execute, don't reason" applies to ALL minted parse-rules — including the Critic's own proposed pin literals in finding fix-prose. If the proposed-fix prose includes a regex literal targeting concrete file coordinates (e.g., "L185", "step 2.1.", `<helper>.py:NNN`), pre-execute that regex against the actual coordinates AT /critique time and report observed shape — quote the command + output in the finding's Evidence sub-section, so the Builder can re-run the empirical step rather than trust the LLM finding-prose blindly. Proposed rule-ID: `APED-1` per -D-suffix convention N=9 → N=10 stable (RSAD-1 + EPGD-1 + SCPD-1 + RPCD-1 + TPHD-1 + LAYER-EVID-1 + FBCD-1 + PTFCD-1 + PTFFD-1 + APED-1).
...
Read `<vault>/triage.md` to confirm the mode before flagging vault-shape issues.

## Output format

Produce a complete critique.md ready to drop into `<vault>/slices/slice-NNN-<name>/critique.md`. Use this exact shape:
