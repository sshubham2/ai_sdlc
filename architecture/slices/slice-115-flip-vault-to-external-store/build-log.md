...
- 2026-06-05 07:30 BUILD: A4 reconnaissance — APED-1 inventory split confirmed: 65 convert (operational-reference + architecture/) vs 62 keep (37 git-pathspec all in code-review/SKILL.md + 25 diagnose-out). Prototyped a line+value-targeted conversion script (works: 52 skill/agent refs convert cleanly; INSTALL/README correctly OUT of ADR-105 <vault>/ scope). REVERTED the prototype to keep a clean A3 checkpoint — the careful remaining A4 work (the code-review/SKILL.md git-pathspec same-value-collision handling [slice-113 un-ratcheted precedent], the slice/SKILL.md M5 line-removal, and the inventory ratchet re-pin verified against --strict) plus the high-stakes Phase B physical flip warrant a focused, unhurried execution rather than a marathon-tail rush. Resume plan captured in milestone.md.
