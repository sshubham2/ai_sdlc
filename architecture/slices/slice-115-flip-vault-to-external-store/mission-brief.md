...
1. **Config-driven flip is live; suite green (location-agnostic) + external resolution verified at process level.** The per-project config at `$GIT_COMMON_DIR/aisdlc/vault-root` holds the absolute external vault path; at process level `tools/_vault_paths.VAULT_ROOT` resolves external and `tools/_vault_git.vault_is_external(repo_root)` returns `True` (the non-default-resolution INFO line fires). The full suite passes as the **location-AGNOSTIC** contract (each test pins its own fixture per slice-110/ADR-101 — in-process `pytest` cannot see a config-driven external `VAULT_ROOT` due to the consumer-freeze cascade; meta-Critic m-add-3); **live-external** resolution + writes are verified separately via the process-level `VAULT_ROOT`/`vault_is_external` checks + the Charter live-fire writes, NOT the unit suite.
...
3. **R-32.a drained — active-slice-folder bootstrap settled.** Active-folder writes resolve to the external store; the op-gate's `OP_DEFERRED_TO_FLIP` bucket is driven to ∅ by **reclassifying** post-flip active-folder writes out of `DEFERRED` (per-slice non-contended ⇒ safe direct external writes — prose `<vault>/` conversion alone does NOT empty it), **re-pinning `_OP_CLASS_FLOOR` 11→0**, and **wiring `--op-gate --strict` into `/build-slice` + `/validate-slice` pre-finish + a shippability row** (an un-drained or un-enforced bucket is a violation, not a waiver — AP-12/AP-18); a brand-new slice can be scaffolded by `/slice` and built by `/build-slice` post-flip.
...
- [ ] **No data loss on the move** — migrate the **full branch `architecture/` tree** (carries ADR-107 + the R-32 risk-register edit + any append, not just `slices/slice-115/` — B1); **full-manifest verify** (file-count equality + per-file SHA-256, NOT a sample — B3) byte-faithful (no EOL churn) before deleting the in-tree copy
- [ ] **No sibling slice in-flight at flip time (DATA-LOSS guard)** — `tools/stranded_slice_audit --json` reports no `BRANCHLESS_IN_FLIGHT` AND no live `slice/*` (minus self); NOT bare `git branch --list` (blind to the untracked branchless scaffold that step-5's `git rm --cached architecture/` + physical-remove would destroy — M3 + meta-Critic M3-adj). Else migrate/quiesce the sibling first
- [ ] **Documented, tested reversibility** — the move-back path (relocate files in, unset `aisdlc/vault-root`, restore tracking, revert prose) is documented and exercised at least once; `cheap` holds only within the same build before post-flip external writes diverge (ADR-107 §Reversibility)
