...
- [ ] **No sibling slice in-flight at flip time** — `git branch --list 'slice/*'` minus self == ∅ (or documented quiesce + rebase-before-merge); a pre-untrack branch would re-introduce tracked `architecture/` at its merge and silently un-do the flip (M3)
