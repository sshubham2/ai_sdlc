...
- **Change**: reclassify post-flip active-folder writes OUT of `DEFERRED` → `OP_OUT_OF_SCOPE` (per-slice non-contended ⇒ safe direct external writes), **keyed on the SINK** (`_ACTIVE_FOLDER_RE.search(sink)`, placed ABOVE the in-loop-skill check at ~L681 — else in-loop active-folder writes fall through to `OP_UNROUTED`; meta-Critic M-add-1). **Re-pin BOTH floors** (a reclass that moves ops between gate-visible buckets must re-pin both, else the destination gains silent slack — meta-Critic M-add-1/AP-12): `_OP_CLASS_FLOOR[OP_DEFERRED_TO_FLIP]` 11→0 AND `_OP_CLASS_FLOOR[OP_OUT_OF_SCOPE]` 23→(23+reclassified count, ~34), each with a mutation-proven non-vacuity test (a 1-op silent demotion into OUT_OF_SCOPE must red). Re-pin the converted-prose ratchet for classes 4–6. Wire `--op-gate --strict` into `/build-slice` Step 6 + `/validate-slice` pre-finish + a `shippability.md` row (no skill consumes the gate today).
...
0. **Quiesce precondition (Critic M3 + meta-Critic severity-adj — a DATA-LOSS path)**: the flip requires **zero other in-flight slices** at flip time, in BOTH classes `stranded_slice_audit` distinguishes: (a) a live `slice/*` branch (predating the untrack) would re-introduce tracked `architecture/` at its `/commit-slice --merge` (silently un-doing the flip) + its in-tree vault writes never reach external; (b) a **`BRANCHLESS_IN_FLIGHT`** slice — a scaffolded `architecture/slices/slice-NNN/` folder with a non-terminal milestone and NO `slice/*` ref (slice-092 / [[decisions/ADR-084]] / R-31) — is **untracked under `architecture/`**, so the full-branch-tree migration (step 3, tracked-content-only) does NOT carry it and step-5's `git rm -r --cached architecture/` + physical-remove would **destroy its only copy**. Guard: `tools/stranded_slice_audit --json` MUST report no `BRANCHLESS_IN_FLIGHT` entry AND no live `slice/*` branch (minus `slice/115`) — NOT the weaker bare `git branch --list` (which is blind to class (b); meta-Critic M3 adj). Currently satisfiable — the slice-115 `stranded_slice_audit` at `/slice` was clean. STOP (and migrate/quiesce the sibling) otherwise.
...
3. **Rebase `slice/115` onto master FIRST**, then **migrate the ENTIRE branch `architecture/` tree** → `<external>/`, **normalizing text (`.md`/`.txt`) to canonical LF on write** (meta-Critic M-add-2): the in-tree working copy may carry CRLF under `core.autocrlf`, and the external store is a plain dir with NO `.gitattributes` renormalization layer, so a raw byte copy would freeze CRLF into the store where the EOL-agnostic comparators (slice-033/ADR-033) + the post-flip `_vault_write` LF writers (slice-094) + ADR-098's `slice-queue.md` byte-identity precondition all assume LF. LF-normalize-on-write keeps the external store canonical. The rebase brings the `chore(queue): pick slice-115` commit current so the branch's `architecture/` is a strict **superset** of master; migrating the *whole branch tree* (NOT master + a single slice-folder overlay) is load-bearing — it captures every branch-authored vault file OUTSIDE `slices/slice-115/`: **`decisions/ADR-107`**, the R-32-retired `risk-register.md` edit, any `lessons-learned.md` append (Critic B1). Untracked sidecar `.lock` files are NOT in the branch tree and are deliberately NOT migrated (they're per-process locks; meta-Critic note). The external store now = the complete current vault, canonical LF.
