...
5. Version + registration meta-AC (M3; 5-part per /code-review m2): a 5-part atomic bump 0.75.0→0.76.0 across `VERSION` + `plugin.yaml.version` + `pyproject.toml [project].version` (PVFS-1) + `## v0.76.0` changelog header + installed `~/.claude/ai-sdlc-VERSION` (AVFS-1), plus the venv `ai-sdlc-tools` re-install (TVFS-1); a `## v0.76.0` DCE-1 changelog entry forward-synced (MCFS-1) with a `test_v_0_76_0_dce_1_*` entry-pin; PMI-1 + INST-1 enumerate `tools/drift_check_audit.py` and stay green.
