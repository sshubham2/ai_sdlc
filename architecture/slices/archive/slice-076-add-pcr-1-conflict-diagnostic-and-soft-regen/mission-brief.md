...
| 5 | methodology / CAD-1 commit-slice drift | tests/methodology/test_commit_slice_skill_drift.py | test_commit_slice_skill_md_in_repo_byte_equal_installed | PASSING |

<!-- TF-1 status legend: PASSING = test exists and passes today (pinning Phase A scaffolding OR a pre-existing canonical test re-verified post-edit). WRITTEN-FAILING = test existed at Phase B and failed for the right reason (NotImplementedError from skeleton, or missing-prose/missing-anchor that Phase C-F resolved); ALL flipped to PASSING by Phase F. The PASSING status on the CAD-1 row is the canonical re-verify-existing-test annotation per slice-026 / ADR-024 precedent: an existing test exercised by this slice but not authored by it; verified continuing-to-pass after the SKILL.md edit at Phase E. The original AC5 "end-to-end regression (manual)" row was removed at Phase G — TF-1's PTFCD-1 grammar refuses `(manual)` as a PASSING test path; the end-to-end invariant (full pytest 1036/1036 PASS + shippability 75/75 PASS + 14+ Step-6 audits clean modulo 2 BC-1 defer-with-rationale per slice-074 N=7 cumulative precedent) is captured in build-log.md Phase G Summary § Pre-finish gate. -->
