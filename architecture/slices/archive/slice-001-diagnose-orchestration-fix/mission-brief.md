...
- [ ] `SKILL.md` call-out: `~/.claude/skills/diagnose/` is the install path; edits go to `<HOME>\ai_sdlc\skills\diagnose\` and require an INSTALL.md re-run to take effect (consistent with INST-1)
...
- [ ] Source files updated in `<HOME>\ai_sdlc\skills\diagnose\` (NOT in `~/.claude/skills/`)
