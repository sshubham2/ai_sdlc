...
- 2026-06-01 (post-code-review HARDENING): applied code-Critic M1/M2/M3 + m1/m3 (advisory→addressed-in-slice; user chose "harden now"). **M1**: `_verdict`→`_is_routed` — a route token cleans ONLY inside a backtick code span or `<!-- route: -->` marker AND not locally negated (2-word look-back); bare/negated mentions ("do NOT use tools.vault_edit append" / "NOT via safe_append_text" / "predates _vault_write") now VIOLATION; the trailing "never a raw `Write`/`Edit`" safety prose stays CLEAN (real-corpus shape preserved — validate-slice:296/user-test:115). **M2**: `_DIRECTIVE_VERBS` +insert/replace/prepend/modify/amend/create; noun-prone verbs (note/record/set/log/mark/put; `register` matches inside `risk-register.md`) DELIBERATELY excluded + documented as a lexicon-bound residual (caught slice:34/221 FPs at build). **M3**: `_REGISTERED_SKILL_EXEMPTIONS` → per-(file,reason) COUNT dict (was pair-set) + `registered_exemption_counts`; an N+1-th marker on a listed file now trips the pin. **m1**: fence tracker CommonMark-correct (`~~~` + char/length; an info-string line cannot close a fence). **m3**: `vault_edit._resolve_in_vault` intentionally rejects `--file .`/empty (vault-root) with exit 2 (was incidental IsADirectoryError).
- 2026-06-01 (post-code-review HARDENING): the m1 fence fix SURFACED triage:179 — a real project-open risk-register write the old naive ``` toggle hid behind triage's malformed nested ```markdown fence (it renders OUTSIDE the template fence per CommonMark: an info-string `:165 ```markdown` is content, `:175 ``` ` closes the `:142` opener). **REVERSED the build-time "drop triage from allowlist" decision** (2026-06-01 01:05 entry — which relied on the fence bug): triage:179 now carries an explicit `<!-- vault-write-safe: project-open-single-shot -->` marker (visible + allowlist-pinned — strictly better for SVW-1's "visible residual" principle than relying on a fence bug). triage:163 stays fence-hidden (separate, still-deferred triage-markdown bug). Counts shift 11→12 exempt / 21→22 sites; reconciled in changelog/build-slice/risk-register (+ installed forward-syncs for triage + build-slice OSDG-1, changelog MCFS-1).
- 2026-06-01 (post-code-review HARDENING): SVW audit re-run CLEAN exit 0 (26 skills, 22 sites: 10 routed, 12 exempted). APED-1 battery extended (+M1 negation/marker ×6, +M2 expanded-verb ×6 + lexicon-residual ×6, +m1 tilde/mixed-char/info-string ×3, +M3 count-pin ×1, + new `tests/methodology/test_vault_edit_cli.py` for m3). Full suite **1367 pass / 0 fail** (was 1335); all guarded-skill drift tests green post-sync.
