...
- Routed append-class prose in `skills/reflect/SKILL.md` (Steps 5 / 5b / 264-row / risk-register-new-entry / methodology-changelog), `skills/reduce/SKILL.md` (Step 8), `skills/repro/SKILL.md` (`:107` shippability append — Critic B2), `skills/validate-slice/SKILL.md` (`:291` risk-register append — meta-Critic M-add-1), `skills/user-test/SKILL.md` (`:115` risk-register append — meta-Critic M-add-1), and `skills/archive/SKILL.md` (archive/_index.md append).
...
| `/validate-slice` | risk-register.md (`:291`, "reality surprise → add immediately"; added per meta-Critic M-add-1) | **append** | route → `vault_edit append` (**M-add-2**: runs per-slice at slice-end → genuinely concurrent → NOT `project-open`; routed, not exempt) |
| `/user-test` | risk-register.md (`:115`, "Surprised/Stuck → Add new risk"; added per meta-Critic M-add-1) | **append** | route → `vault_edit append` |
...
- **The append sites of `/reflect`, `/reduce`, `/repro`, `/archive`, `/validate-slice`, `/user-test` are what this slice routes; the rewrite (RMW) sites of `/reflect`, `/archive`, `/supersede-slice` are the deferred residual.** (The enumeration is now COMPLETE per an independent grep of all 26 skills — first Critic B2 found `/repro`+`/supersede-slice`; meta-Critic M-add-1 found `/validate-slice`+`/user-test`; the Builder re-grep confirmed no 5th site. `reflect:386` "Update risk-register.md to reflect that nuance" is descriptive example-prose (no directive, no backtick) — correctly excluded.)
