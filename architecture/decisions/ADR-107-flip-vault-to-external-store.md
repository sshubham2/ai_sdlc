...
- **Class 4–6 carve-out prose** converts to `<vault>/`. `OP_DEFERRED_TO_FLIP` → ∅ is delivered NOT by that prose conversion (it leaves the classifier output unchanged — `_ACTIVE_FOLDER_RE` matches under both roots) but by **reclassifying** post-flip active-folder writes out of `DEFERRED` → `OP_OUT_OF_SCOPE` (per-slice, non-contended ⇒ safe direct external writes), re-pinning `_OP_CLASS_FLOOR` 11→0, and wiring the gate into `/build-slice` + `/validate-slice` pre-finish (design.md; Critic B2/AP-12).
- **`diagnose-out/` scope reconciliation (vs ADR-085)**: ADR-085 §Decision states the move scope as "`architecture/` **+ `diagnose-out/`**". This slice deliberately does **NOT** inherit the `diagnose-out/` leg — `diagnose-out/` is already gitignored (`.gitignore:20`), so it carries **no** R-32 shared-mutable-git-conflict class, and slice-105 ([[decisions/ADR-095]]) decoupled the slice loop from it in both directions. Relocating `diagnose-out/` would retire no part of R-32; it is a separate, optional future slice, not a flip residual. Recorded here so the narrowing is a written decision, not an inferred omission.
- **Class 7 (diagnose-out)** carve-out prose stays concrete (no `<diagnose-out>` seam minted — slice-112 B5).
- **EOL canonicalization (meta-Critic M-add-2)**: the external store is a plain directory with no `.gitattributes` renormalization layer, and only `architecture/slice-queue.md` was in-tree EOL-pinned (slice-109/ADR-098 for the CAS byte-identity precondition). The migration therefore **normalizes text to canonical LF on write**, so the external store is uniformly LF — matching the EOL-agnostic comparators (ADR-033), the post-flip `_vault_write` LF writers (slice-094), and **re-homing ADR-098's `slice-queue.md` byte-identity precondition** (the external `slice-queue.md` is LF, so the slice-109 CAS base-capture/compare stays byte-stable without the now-inapplicable `.gitattributes` pin). The full-manifest verify (design.md §Invariant) compares LF-normalized content, so the CRLF→LF normalization is verified, not a false mismatch.
- **Self-referential bootstrap**: slice-115 is the first slice to live through the flip — its own active folder migrates to the external store mid-build (design.md §The flip sequence), and the migration carries the **full branch `architecture/` tree** so ADR-107 itself is not lost (Critic B1).
- **R-32 retires** at the move.

## Reversibility

**Cheap — within a bounded window (Critic M4).** `tools/_vault_flip.py --rollback` is the scripted inverse: move `<external>/*` back to `architecture/`, unset the `aisdlc/vault-root` config, reverse the `git rm --cached` (re-track) + remove the `.gitignore` entry; the prose/guard changes revert via `git revert`. **Hard precondition for "cheap"**: rollback is a pure inverse only **within the same build, BEFORE post-flip external writes diverge**. Once `/validate-slice` / `/reflect` (or any later slice) write content that exists ONLY in the external store, rollback must additionally **reconcile that external-only content back in-tree** — it is no longer a pure inverse, and "no data loss" then depends on that reconciliation. Tagged `cheap` consistent with the R-32 register entry on that bounded-window basis (the rollback tool + the full-manifest verify keep it cheap rather than a manual 1091-file scramble); outside the window it is "moderate." The WIRE-1 round-trip test asserts the **complete** file set restores, not a sample.
