...
- **The audit is static — it is the LLM-prose-inspection shape [[ADR-029]] rejected, and that is acceptable here for a specific reason (Critic B1).** ADR-029/BCI-1 rejected hardening/inspecting LLM-executed vault-write prose in favor of a deterministic downstream gate over a tracked canonical fixture. SVW-1 cannot take that path because there is **no oracle artifact**: an arbitrary `vault_edit append` of LLM-authored content (a fresh risk entry, a lessons bullet) has no reproducible expected-value to diff against, unlike `build-checks.md`'s `canonical_project_checks.md` fixture. So SVW-1's guarantee is scoped to the **prose-detection surface** (no `skills/*/SKILL.md` *prescribes* a raw bypass — drift-prevention, the CAD-1/OSDG-1 philosophy), explicitly NOT a fail-closed completeness guarantee over runtime writes. It cannot prove Claude obeys the prose at runtime (the R-2 class — structurally unreachable by a static audit). Runtime-write safety rests on the wrapper being USED + the pre-flip git/PCR backstop + the cooperative model ([[ADR-067]]), not on this audit. Acceptable; over-claiming "fail-closed completeness over writes" would not have been.
- **The exemption marker is constrained to a closed reason-enum + a pinned allowlist (Critic M3).** A free-text `<!-- vault-write-safe: <reason> -->` is the per-line-`# noqa` silent-bypass vector this slice's own R-7 must-not-defer forbids — VWS-1 (ADR-086) deliberately used a singular module-name exemption, not per-line suppressions. SVW-1 therefore (a) accepts `<reason>` only from a closed set `{deferred-rmw, project-open-single-shot}` (unknown reason → VIOLATION) and (b) pins a closed allowlist of exempt site locations (the slice-041 `_REGISTERED_INSTALLED_READERS` shape) so a NEW exemption added by a future skill edit trips a pin regression and forces review — keeping "visible residual" enforced, not self-asserted.
