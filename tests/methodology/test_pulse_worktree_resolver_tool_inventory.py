...
def test_pulse_worktree_resolver_in_canonical_tools_plugin_manifest_install_md_at_l22_and_l166():
    ...
    assert "40" in l22, (
        f"INSTALL.md L22 missing post-bump tool-count `40` (…→38→39→40, two-site; slice-100 +1 vault_flip_readiness_audit); "
        f"actual line content: {l22!r}"
    )
    assert "40" in l166, (
        f"INSTALL.md L166 missing post-bump tool-count `40`; actual line content: {l166!r}"
    )
