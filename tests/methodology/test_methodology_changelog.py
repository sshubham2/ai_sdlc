...
def test_v_0_74_0_pcr_2a_entry_present_in_repo():
    """methodology-changelog v0.74.0 / PCR-2a vault-claim sub-mechanism
    entry-pin (content-bearing per slice-076 PCR-1 precedent).

    Asserts the load-bearing substring anchors:
      (a) `## v0.74.0` dated header
      (b) `PCR-2a` rule reference (the new RULE-ID this entry mints)
      (c) `ADR-071` reference
      (d) `parallel-conflict-resolution` canonical phrase (family axis)
      (e) `mints a new rule` (refines PCR-1 in-place; supersedes nothing
          per ADR-071 frontmatter)
      (f) `5-part PMI-1 atomic bump` (canonical anchor)
      (g) `Rule reference` literal (META-1 mandatory enforcing-assertion)
    """
    in_repo = read_file("methodology-changelog.md")
    assert "## v0.74.0" in in_repo, (
        "in-repo methodology-changelog.md missing v0.74.0 entry header — "
        "slice-078 PCR-2a entry was not added or was lost"
    )
    body = _extract_version_body(in_repo, "0.74.0")
    assert "PCR-2a" in body, (
        "v0.74.0 entry body missing the 'PCR-2a' rule reference — "
        "entry-pin broken at the rule-reference layer"
    )
    assert "ADR-071" in body, (
        "v0.74.0 entry body must reference ADR-071 (the new ADR minting PCR-2a)"
    )
    assert "parallel-conflict-resolution" in body, (
        "v0.74.0 entry body missing the 'parallel-conflict-resolution' "
        "canonical phrase anchor — PCR-2a's family axis name"
    )
    assert "mints a new rule" in body, (
        "v0.74.0 entry must state 'mints a new rule' (PCR-2a refines PCR-1's "
        "class taxonomy in-place; second rule on the PCR-N family axis; "
        "supersedes nothing per ADR-071)"
    )
    assert "5-part PMI-1 atomic bump" in body, (
        "v0.74.0 entry body missing the '5-part PMI-1 atomic bump' anchor"
    )
    assert "Rule reference" in body, (
        "v0.74.0 entry missing the literal 'Rule reference' line — "
        "META-1 entry-pin obligation unmet"
    )
...
def test_v_0_76_0_dce_1_entry_present_in_repo():
    """methodology-changelog v0.76.0 / DCE-1 Drift-Check Enforcement gate
    entry-pin (content-bearing per slice-076/078/080 precedent).

    Asserts the load-bearing substring anchors:
      (a) `## v0.76.0` dated header
      (b) `DCE-1` rule reference (the new RULE-ID this entry mints)
      (c) `ADR-073` reference
      (d) `mints a new rule` (supersedes nothing per ADR-073 frontmatter)
      (e) `5-part PMI-1 atomic bump` (canonical anchor)
    """
    in_repo = read_file("methodology-changelog.md")
    assert "## v0.76.0" in in_repo, (
        "in-repo methodology-changelog.md missing v0.76.0 entry header — "
        "slice-081 DCE-1 entry was not added or was lost"
    )
    body = _extract_version_body(in_repo, "0.76.0")
    assert "DCE-1" in body, (
        "v0.76.0 entry body missing the 'DCE-1' rule reference — "
        "entry-pin broken at the rule-reference layer"
    )
    assert "ADR-073" in body, (
        "v0.76.0 entry body must reference ADR-073 (the new ADR minting DCE-1)"
    )
    assert "mints a new rule" in body, (
        "v0.76.0 entry must state 'mints a new rule' (DCE-1 supersedes nothing per ADR-073)"
    )
    assert "5-part PMI-1 atomic bump" in body, (
        "v0.76.0 entry body missing the '5-part PMI-1 atomic bump' anchor"
    )


def test_v_0_76_0_dce_1_shippability_consumer_propagation():
    """RPCD-1/SCPD-1 consumer-reference propagation: the DCE-1 consumer
    reference MUST propagate into `architecture/shippability.md` so the
    slice-081 DCE-1 gate can never silently regress.

    The DCE-1 gate row is located by its entry-pin test name (NOT the bare
    slice-081 slug — that also matches the BFRD-1 repro row #86).
    ...
    assert anchor in catalog, (
        ...
        "entry-pin test by canonical name (BC-PROJ-10 paired-pin schema)"
    )
    row_start = catalog.rfind("\n| ", 0, catalog.find(anchor)) + 1
    row_end = catalog.find("\n| ", catalog.find(anchor))
    row = catalog[row_start:row_end] if row_end > 0 else catalog[row_start:row_start + 8000]
    assert "DCE-1" in row, "DCE-1 gate row must cite DCE-1 (the new RULE-ID)"
    assert "ADR-073" in row, "DCE-1 gate row must cite ADR-073 (the new ADR)"
    assert "test_v_0_76_0_dce_1_shippability_consumer_propagation" in row, (
        "DCE-1 gate row must cite the shippability-consumer-propagation test "
        "function by canonical name (BC-PROJ-10 paired-pin schema)"
    )
    assert "drift_check_audit" in row, (
        "DCE-1 gate row must reference 'drift_check_audit' (the touched module) "
        "for catalog-runner discoverability"
    )
...
def test_v_0_78_0_pfs1_entry_present_in_repo():
    """methodology-changelog v0.78.0 / PFS-1 entry-pin (content-bearing per
    slice-073/076/077/081 precedent).

    Asserts the load-bearing substring anchors:
      (a) `## v0.78.0` dated header
      (b) `PFS-1` rule reference (the new RULE-ID)
      (c) `ADR-080` reference (the ADR minting PFS-1)
      (d) `review-context` family-axis phrase
      (e) `mints a new rule`
      (f) `5-part PMI-1 atomic bump`
      (g) `OSDG-1 guarded-set extended` (M3a — the OSDG-1 membership-extension discharge)
      (h) `Rule reference` literal (META-1 mandatory enforcing-assertion)
    """
    in_repo = read_file("methodology-changelog.md")
    assert "## v0.78.0" in in_repo, (
        "in-repo methodology-changelog.md missing v0.78.0 entry header — "
        "slice-088 PFS-1 entry was not added or was lost"
    )
    body = _extract_version_body(in_repo, "0.78.0")
    assert "PFS-1" in body, (
        "v0.78.0 entry body missing the 'PFS-1' rule reference (new rule on the "
        "review-context axis)"
    )
    assert "ADR-080" in body, (
        "v0.78.0 entry body must reference ADR-080 (the new ADR minting PFS-1)"
    )
    assert "review-context" in body, (
        "v0.78.0 entry body missing the 'review-context' family-axis phrase"
    )
    assert "mints a new rule" in body, (
        "v0.78.0 entry must state 'mints a new rule' (PFS-1; supersedes nothing per ADR-080)"
    )
    assert "5-part PMI-1 atomic bump" in body, (
        "v0.78.0 entry body missing the '5-part PMI-1 atomic bump' anchor"
    )
    assert "OSDG-1 guarded-set extended" in body, (
        "v0.78.0 entry must record the OSDG-1 guarded-set extension to "
        "design-slice/critique/critique-review (M3a discharge)"
    )
    assert "Rule reference" in body, (
        "v0.78.0 entry missing the literal 'Rule reference' line — META-1 entry-pin obligation unmet"
    )


def test_v_0_78_0_pfs1_shippability_consumer_propagation():
    """RPCD-1/SCPD-1 consumer-reference propagation: the PFS-1 consumer
    reference MUST propagate into `architecture/shippability.md` (catalog
    row #93) so the slice-088 critical path can never silently regress.
...
def test_v_0_80_0_vws_1_entry_present_in_repo():
    """methodology-changelog v0.80.0 / VWS-1 entry-pin (content-bearing per
    slice-073/076/077/081/088/095 precedent). Asserts the load-bearing anchors:
      (a) `## v0.80.0` dated header
      (b) `VWS-1` rule reference (the new RULE-ID)
      (c) `ADR-086` reference (the ADR minting VWS-1)
      (d) `vault-write-safety` family-axis phrase
      (e) `mints a new rule`
      (f) `5-part PMI-1 atomic bump`
      (g) `Rule reference` literal (META-1 mandatory enforcing-assertion)
    """
    in_repo = read_file("methodology-changelog.md")
    assert "## v0.80.0" in in_repo, (
        "in-repo methodology-changelog.md missing v0.80.0 entry header — "
        "slice-094 VWS-1 entry was not added or was lost"
    )
    body = _extract_version_body(in_repo, "0.80.0")
    assert "VWS-1" in body, (
        "v0.80.0 entry body missing the 'VWS-1' rule reference (new rule on the "
        "vault-write-safety axis)"
    )
    assert "ADR-086" in body, (
        "v0.80.0 entry body must reference ADR-086 (the ADR minting VWS-1)"
    )
    assert "vault-write-safety" in body, (
        "v0.80.0 entry body missing the 'vault-write-safety' family-axis phrase"
    )
    assert "mints a new rule" in body, (
        "v0.80.0 entry must state 'mints a new rule' (VWS-1; supersedes nothing per ADR-086)"
    )
    assert "5-part PMI-1 atomic bump" in body, (
        "v0.80.0 entry body missing the '5-part PMI-1 atomic bump' anchor"
    )
    assert "Rule reference" in body, (
        "v0.80.0 entry missing the literal 'Rule reference' line — META-1 entry-pin obligation unmet"
    )


def test_v_0_80_0_vws_1_shippability_consumer_propagation():
    """RPCD-1/SCPD-1 consumer-reference propagation: the VWS-1 consumer reference
    MUST propagate into `architecture/shippability.md` (catalog row #104) so the
    slice-094 critical path can never silently regress.
    ...
    row_start = catalog.find("slice-094-harden-vault-write-safety")
    row_end = catalog.find("\n| ", row_start)
    row = catalog[row_start:row_end] if row_end > 0 else catalog[row_start:row_start + 8000]
    assert "VWS-1" in row, "shippability.md row #104 must cite VWS-1 (the new RULE-ID)"
    assert "ADR-086" in row, "shippability.md row #104 must cite ADR-086 (the ADR)"
    assert "test_v_0_80_0_vws_1_entry_present_in_repo" in row, (
        "shippability.md row #104 must cite the entry-pin test function by canonical name"
    )
    assert "test_v_0_80_0_vws_1_shippability_consumer_propagation" in row, (
        "shippability.md row #104 must cite the shippability-consumer-propagation test function"
    )
