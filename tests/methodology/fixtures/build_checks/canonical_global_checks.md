...
## BC-GLOBAL-5 — Capturing subprocess output in text mode must pass encoding="utf-8", never the host locale code page

**Severity**: Important
**Applies to**: **
**Promoted from**: ai_sdlc slice-090-fix-pcr-git-subprocess-cp1252-decode (2026-05-31) — git `subprocess.run(..., text=True)` calls with no `encoding=` decoded git's UTF-8 output as cp1252 on Windows; a byte undefined in cp1252 raised `UnicodeDecodeError` inside subprocess's pipe-reader thread, which `subprocess.run` swallows (returns `stdout=None`), silently corrupting the captured result. Cousin to the stdout-side cp1252 class (set the process stdout encoding) which recurred repeatedly.
**Trigger keywords**: subprocess, popen, encoding, decode, cp1252, locale, capture_output
...
**Check**: When code runs `subprocess.run` / `Popen` / `check_output` (or any language's equivalent) that CAPTURES a child process's output in TEXT mode, it MUST specify an explicit UTF-8 codec (`encoding="utf-8"` in Python). Without it, the runtime decodes the captured pipe using the host's locale/ANSI code page (cp1252 on Windows, locale-dependent elsewhere); a byte undefined in that code page (e.g. from emoji or non-Latin content the child emits) raises a decode error — in CPython this raises INSIDE the pipe-reader thread, where `subprocess.run` does NOT re-raise it and instead returns `None`/truncated output, so the caller proceeds on corrupted data (silent failure, not a clean crash). Byte-mode captures that never decode (bytes used only for an error repr) are exempt. Keep error handling strict — a lossy `errors="replace"` policy silently mojibakes real data and re-opens the same data-integrity hole.
